        ...
        <div class="mainBody flex1">
            ...
            <div class="titleBar">
                ...
                <div class="floatLeft">
                    <%-- <span>选择取样时间</span> --%>
                    <span>
                        <input  id="beginDate"  name="beginDate"  type="text"  placeholder="点击选择取水样品开始时间"/>
                    </span>
                </div>
                <div class="floatLeft">
                    <%-- <span>选择取样时间</span> --%>
                    <span>
                        <input id="endDate"  name="endDate"  type="text"  placeholder="点击选择取水样品结束时间"/>
                    </span>
                </div>
            ...
            <!-- 复选框 -->
            <div class="titleBar displayFlex" id="titleBar2" ></div>
            </form>
            <%-- 表单 --%>
            <!-- title选项卡 结束 -->
            <!-- 图表集合 开始-->
...
<script>
    ...
    $.ajax({
        ...
        success: function(data){
            var str01 = ' <option value="-1">选择取水样品</option> ';
            // 遍历样品类型
...
<script>
    $(".submit").click(function(){
    	var arr = [];
        $("#titleBar2 label").each(function(){
            var zhi = $(this).attr("checked");
            if(zhi == "checked"){
                var zhi1 = $(this).prev().attr("value");
                arr.push(zhi1);
            }
        });
        $("#sampleValue").remove();
        $("#submitForm").append('<input type="text" id="sampleValue" name="sampleValue" value="'+arr.join(",")+'" style="display:none;"/>');
    	 // 允许生成多个柱状图
		// $(".charts").empty();
        // 查询数据
        $.ajax({
            type: 'POST',
            url: '${ctx }/dateSort.html',
