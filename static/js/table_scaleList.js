...
function useTable_addInfo(json){
    ...
    //表内容
    var dataInfo = useTable_addInfo_changeForm(json.scaleList);
    ...
    $(".jsgrid-edit-button").click(function(){
        // //点击编辑按钮后
           ...
           window.location.href = ctx +"/scaleUpdate.jsp?scaleId="+a;
...
function returnObj(elem){
    var obj = {};
    obj.scaleId = $(elem).parents("tr").find("input").eq(0).val();
    obj.scaleName = $(elem).parents("tr").find("input").eq(1).val();
    // console.log(obj)
    return obj;
}
